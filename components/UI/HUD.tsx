...
import { Heart, Zap, Trophy, MapPin, Diamond, Rocket, ArrowUpCircle, Shield, Activity, PlusCircle, Play, Loader2, Volume2, VolumeX } from 'lucide-react';
...
export const HUD: React.FC = () => {
  const { score, lives, maxLives, collectedLetters, status, level, restartGame, startGame, gemsCollected, distance, isImmortalityActive, speed, isMuted, toggleMute } = useStore();
  const [menuImage, setMenuImage] = useState<string | null>(null);
  const [isGenerating, setIsGenerating] = useState(false);
  const target = ['G', 'A', 'L', 'A', 'X', 'Y'];

  useEffect(() => {
    if (status === GameStatus.MENU && !menuImage && !isGenerating) {
        generateCover();
    }
  }, [status]);
  ...
  const generateCover = async () => {
    try {
        ...
        const prompt = "A vibrant synthwave retro-futuristic runner sprinting through a neon-grid galaxy. The sky is filled with a giant colorful sun and nebula clouds. Big bold glowing neon text in the center says 'GALAXY RUNNER'. Professional high-quality digital art style, cyan and magenta theme.";
        ...
        for (const part of response.candidates[0].content.parts) {
            if (part.inlineData) {
                ...
                setMenuImage(`data:image/png;base64,${base64}`);
                break;
            }
        }
    } catch (error) {
        console.error("Failed to generate galaxy runner cover:", error);
        // Fallback to a solid color if generation fails
        setMenuImage("https://www.gstatic.com/aistudio/starter-apps/gemini_runner/gemini_runner.png"); 
    } finally {
                ...
                <div className="relative w-full aspect-square bg-gray-950 flex items-center justify-center">
                     {isGenerating ? (
                        <div className="flex flex-col items-center">
                            ...
                            <p className="text-cyan-400 font-mono text-sm animate-pulse">GENERATING GALAXY...</p>
                        </div>
                     ) : (
                        <img 
                            src={menuImage || "https://www.gstatic.com/aistudio/starter-apps/gemini_runner/gemini_runner.png"} 
                            alt="Galaxy Runner Cover" 
  ...
  if (status === GameStatus.GAME_OVER) {
      return (
          <div className="absolute inset-0 bg-black/90 z-[100] text-white pointer-events-auto backdrop-blur-sm overflow-y-auto">
              <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                <h1 className="text-4xl md:text-6xl font-black text-white mb-6 drop-shadow-[0_0_10px_rgba(255,0,0,0.8)] font-cyber text-center">MISSION FAILED</h1>
                ...
                <div className="grid grid-cols-1 gap-3 md:gap-4 text-center mb-8 w-full max-w-md">
                    <div className="bg-gray-900/80 p-3 md:p-4 rounded-lg border border-gray-700 flex items-center justify-between">
                        ...
                        <div className="text-xl md:text-2xl font-bold font-mono">{level} / 5</div>
                    </div>
                    <div className="bg-gray-900/80 p-3 md:p-4 rounded-lg border border-gray-700 flex items-center justify-between">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-purple-900/90 to-black/95 z-[100] text-white pointer-events-auto backdrop-blur-md overflow-y-auto">
            <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <p className="text-cyan-300 text-sm md:text-2xl font-mono mb-8 tracking-widest text-center">
                    YOU HAVE BECOME THE MASTER OF THE COSMOS
                </p>
    ...
    <div className={containerClass}>
        <div className="flex justify-between items-start w-full">
            ...
            <div className="flex items-center space-x-3 md:space-x-4">
                <button 
                    onClick={toggleMute}
                    className="pointer-events-auto p-2 bg-black/40 hover:bg-black/60 rounded-full border border-white/20 transition-colors"
                    title={isMuted ? "Unmute" : "Mute"}
                >
                    {isMuted ? <VolumeX className="text-red-500 w-6 h-6" /> : <Volume2 className="text-cyan-400 w-6 h-6" />}
                </button>
